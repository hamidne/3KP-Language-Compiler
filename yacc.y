...
%left mul_t devide
%%
Program : program ID ';' DecList '{'SList  '}' '.' {printf("reduced FROM program ID ';' DecList '{'SList  '}' '.' to Program \n");}
        ;

DecList : Dec  DecList {printf("reduced FROM Dec  DecList TO DecList \n");}
        |              {printf("reduced FROM  TO DecList \n");}
        ;

Dec : VarDecs ';'      {printf("reduced FROM VarDecs ';' TO Dec \n");}
    | FuncDecs         {printf("reduced FROM FuncDecs TO Dec \n");}
    ;
FuncDecs : FuncDec FuncDecs {printf("reduced FROM FuncDec FuncDecs TO FuncDecs \n");}
         |                  {printf("reduced FROM  TO FuncDecs \n");}
         ;

VarDecs : VarDec           {printf("reduced FROM VarDec TO VarDecs \n");}
        | VarDec VarDecs   {printf("reduced FROM VarDec VarDecs TO VarDecs \n");}
        ;

VarDec : Type IDDList      {printf("reduced FROM Type IDDList TO VarDec \n");}
       ; 

Type : integer  {printf("reduced FROM integer  TO Type \n");}
...
IDDim: ID                        {printf("reduced FROM ID  TO IDDim \n");}
     | IDDim '['integerNumber']' {printf("reduced FROM IDDim '['integerNumber']'  TO IDDim \n");}
     ;

IDDList : IDDim                  {printf("reduced FROM IDDim  TO IDDList \n");}
        | IDDim ',' IDDList      {printf("reduced FROM IDDim ',' IDDList   TO IDDList \n");}
        ;

IDList : ID                      {printf("reduced FROM ID  TO IDList \n");}    
       | ID ',' IDList           {printf("reduced FROM ID ',' IDList  TO IDList \n");}
       ;

FuncDec :Type ID '(' ArgsList ')' '{' SList '}' ';' {printf("reduced FROM Type ID '(' ArgsList ')' '{' SList '}' ';' TO FuncDec \n");}
        ;

ArgsList :ArgList {printf("reduced FROM ArgList TO ArgsList \n");}
         |        {printf("reduced FROM  TO ArgsList \n");}
         ;

ArgList : Arg               {printf("reduced FROM ArgList TO Arg \n");}
        | Arg ';' ArgList   {printf("reduced FROM Arg ';' ArgList TO Arg \n");}
        ;

Arg : Type IDList           {printf("reduced FROM Arg TO Type IDList  \n");}
    ;

SList :Stmt ';' SList       {printf("reduced FROM Stmt ';' SList TO SList  \n");}
      |                     {printf("reduced FROM  TO SList  \n");}
      ;

Stmt : Exp                              {printf("reduced FROM Exp TO Stmt  \n");}
...
valfor:to_t                                      {printf("reduced FROM to_t  TO valfor  \n");}
      |down_t to_t                               {printf("reduced FROM down_t to_t   TO valfor  \n");}
      ;
Range :Exp DOnoghte Exp                               
      ;                           

Cases :Case                                       {printf("reduced FROM Case TO Cases  \n");}
      | Case Cases                                {printf("reduced FROM Case Cases TO Cases  \n");}
      ;

Case : case_t Exp ':' Block                      {printf("reduced FROM case_t Exp ':' Block TO Case \n");}
     | case_t Range ':' Block                    {printf("reduced FROM case_t Range ':' Block TO Case \n");}
     ;
  
Logic :and_t                            {printf("reduced FROM and_t TO Logic \n");}
...
Aop : add_t                            {printf("reduced FROM add_t TO Aop \n");}
...
ExpList :ExpPlus                        {printf("reduced FROM ExpPlus TO ExpList \n");}
        |                               {printf("reduced FROM  TO ExpList \n");}
        ;

ExpPlus :Exp                           {printf("reduced FROM Exp TO ExpPlus \n");}
        | Exp ',' ExpPlus              {printf("reduced FROM Exp ',' ExpPlus TO ExpPlus \n");}
        ;

IDD :ID                               {printf("reduced FROM Exp TO IDD \n");} 
    | IDD '[' Exp ']'                 {printf("reduced FROM IDD '[' Exp ']' TO IDD \n");} 
    ;

lvalue : ID                           {printf("reduced FROM ID TO lvalue \n");} 
       | IDD                          {printf("reduced FROM IDD TO lvalue \n");} 
       ;

Exp :integerNumber                    {printf("reduced FROM integerNumber TO Exp \n");}
    |RealNumbe_t
    ...
    ; 
Block : '{' SList '}'                 {printf("reduced FROM '{' SList '} TO Block \n");}
